...
(function($) {
    ...
    $.fn.iconPicker = function( options ) {
        ...
        var iconExt = new Array("icon-new-save", "icon-new-clone", "icon-new-document", "icon-new-search", "icon-new-deletefolder", "icon-new-newfolder", "icon-new-recycle", "icon-new-add", "icon-new-addpeers", "icon-new-addchild", "icon-new-delete", "icon-new-copy", "icon-new-save", "icon-new-move", "icon-new-movedown", "icon-new-moveup", "icon-new-refresh", "icon-new-edit", "icon-new-empty", "icon-new-log", "icon-new-initialize", "icon-new-home", "icon-new-download", "icon-new-upload", "icon-new-management", "icon-new-dontpass", "icon-new-user", "icon-new-role", "icon-new-baobiao");
        var settings = $.extend({
        ...
        return this.each( function() {
	        ...
	        function createUI($element){
	        	...
	        	$(".ip-control .btn",$popup).click(function(e){
	                ...
	                if(start_index+per_page<=240){//根据图片个数多少来设置
	        ...
	        function showList($element,arrLis,arrlistext){
	        	$ul=$("<ul>");
	        	
	            //for (var i in arrLis) 会循环到深层次_proto中去
	        	if (arrlistext) {
	        	    for (var i = 0; i < arrlistext.length; i++) {
	        	        $ul.append("<li><a href=\"#\" ext=true title=" + arrlistext[i] + "><span class=\"glyphicon " + arrlistext[i] + "\" style='width:14px;height:15px;'></span></a></li>");
	        	    };
	        	}
